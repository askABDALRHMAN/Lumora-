...
export const GalleryManagement: React.FC = () => {
  ...
  const resetForm = () => {
    setFormData({ title:'', titleEn:'', description:'', descriptionEn:'', image:'', category:'' });
  ...
  const handleUpdate = () => {
    if (!editingItem || !formData.title || !formData.titleEn || !formData.description || !formData.descriptionEn) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {galleryItems.map(item => (
          <Card key={item.id} className="hover:shadow-lg transition-shadow">
            <CardHeader>
              <img src={item.image} alt={language==='ar'?item.title:item.titleEn} className="w-full h-48 object-cover rounded-lg"/>
            </CardHeader>
            <CardContent>
              <CardTitle className="mb-2">{language==='ar'?item.title:item.titleEn}</CardTitle>
              <CardDescription className="mb-4">{language==='ar'?item.description:item.descriptionEn}</CardDescription>
              <div className="flex gap-2">
