...
export const FAQManagement: React.FC = () => {
  ...
  const handleAdd = () => {
    if (!formData.question || !formData.questionEn || !formData.answer || !formData.answerEn) {
      toast({
        title: 'خطأ',
  ...
  const handleUpdate = () => {
    if (!editingFAQ || !formData.question || !formData.questionEn || !formData.answer || !formData.answerEn) {
      toast({
        title: 'خطأ',
        description: 'يرجى ملء جميع الحقول المطلوبة',
        variant: 'destructive'
